...
## PROJECT DIRECTIVE (emilkrebs.dev)

> Dominant source of truth for any frontend work in this repository:
> 1. Read `PRODUCT.md` and `DESIGN.md` at the repo root first. DESIGN.md wins on every visual decision; PRODUCT.md wins on voice, content, and product truth.
> 2. This is a redesign in overhaul mode: the incumbent purple/blue dark portfolio is a binding anti-reference, never polish it.
> 3. The committed world is "The Spec Sheet": warm paper `#f7f5f0`, ink `#141310`, one signal accent `#e8450c`, 0px corners everywhere, flat with no shadows, Schibsted Grotesk + IBM Plex Mono + a single Instrument Serif italic accent word per section. All-sharp, light theme only. All imagery sits in hairline plates with mono captions; only the hero portrait is duotone, product screenshots and live previews (iframes) are shown full color as they are; placeholders must be labeled, never faked.
> 4. Never add decoration beyond the defined token-mark rows; never animate on load; never introduce a second accent or a new font.
> 5. The DSL playground (Vite static app, built into `public/playground/`) embeds on /healthstack as a first-party same-origin direct-load iframe, lazy, full color, no consent gate. Its DSL sources are vendored: resync with `npm --prefix playground run sync-dsl`, never edit `playground/src/lang` or `playground/src/samples` by hand (generated output).
