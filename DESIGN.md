...
**Language switcher:** rendered only on the localized pages (`/zh/`, `/zh/story/`), never on the English defaults — the English pages are the fallback, so they carry no switcher. It is a bordered tag in the nav grammar (hairline border, 0px corners, mono, ink-soft, hover ink) so it reads as an action distinct from nav links. Label: EN, returning to the English counterpart (`/zh/` to `/`, `/zh/story/` to `/story/`, where it sits at the right end of the back header).
