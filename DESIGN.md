...
- **event-list:** the timetable of the /story page, set like the Contents block: hairline frame, 0px corners, a mono label row ("Events" / "活动"), then one row per event. Each row is a date column (mono, ink-soft, fixed width so names align like departures on a ferry timetable), the event name (body, medium, ink, signal arrow when it links out), and the city plus a role tag (mono; tag grammar) at the right end. The whole row is the link, to the event's own site; hover is the paper-deep step, as in Contents. Mobile stacks date, name, then city and tag. Dates are authored per locale: "Sep 6-7, 2026" in en, "2026年9月6-7日" in zh (no spaces inside CJK dates: mono tracking turns them into gaps); when only an age is known, the date column states the age ("Age 15" / "15岁"), in the same register as the era periods. No images, no logos: the row is a record, not a gallery.
